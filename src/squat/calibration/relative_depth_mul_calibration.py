...
class RelativeDepthMulCalibrationParams:
    thigh_a: float
    thigh_c: float
    thigh_b: float
    shank_a: float
...
def _fit_multiplicative_bias_model(
    raw_dz_list: list[float],
    len_2d_list: list[float],
    ...
    # target_dz = raw_dz * (a + c * (ref_len - len_2d)) + b
    #           = a * raw_dz + c * raw_dz * (ref_len - len_2d) + b
    mat = np.column_stack([raw, raw * delta_len, np.ones_like(raw)])
...
def fit_relative_depth_mul_calibration(
    ...
    thigh_a, thigh_c, thigh_b = _fit_multiplicative_bias_model(
        raw_dz_list=raw_dz_thigh_list,
        len_2d_list=thigh_len_2d_list,
        target_dz_list=thigh_target.tolist(),
        ref_len=ref_thigh_len,
    )
    shank_a, shank_c, shank_b = _fit_multiplicative_bias_model(
        raw_dz_list=raw_dz_shank_list,
        len_2d_list=shank_len_2d_list,
    ...
    return RelativeDepthMulCalibrationParams(
        thigh_a=thigh_a,
        thigh_c=thigh_c,
        thigh_b=thigh_b,
        shank_a=shank_a,
...
def predict_relative_mul_dz(
    ...
    return raw_dz * (a + c * (ref_len - len_2d)) + b