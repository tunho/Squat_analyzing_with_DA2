...
def main():
    print("=== RAZS 1-Pass vs 2-Pass Comparison (SAMPLES) ===")
    
    # 1. Setup Folders
    sample_root = "/home/lee/exe_est/sample"
    output_dir = "/home/lee/exe_est/comparison_results_sample"
    ...
    # Process all mp4 files in sample root
    video_files = glob.glob(os.path.join(sample_root, "*.mp4"))
    print(f"\nProcessing Samples: Found {len(video_files)} videos")
    ...
    for video_path in video_files:
        ...
        try:
            # Run 1-Pass
            c1, _, issues1, _ = analyzer.process_video(
    ...
    if results_list:
        ...
        report_path = os.path.join(output_dir, "sample_comparison_report.csv")
        df.to_csv(report_path, index=False)
        print(f"\nSample Comparison Report saved to: {report_path}")
        print(df)
    else:
        print("\nNo sample videos processed.")
