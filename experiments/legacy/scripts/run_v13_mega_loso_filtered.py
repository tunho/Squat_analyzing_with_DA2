...
SAVE_DIR = PROJECT_ROOT / "outputs/ml_correction/v13_mega_loso_filtered"
...
def run_experiment():
    ...
    # FILTER: Exclude Fit3D Ex6 (Abduction) as per user suggestion
    df = full_df[~((full_df['subject_id'].str.contains('fit3d')) & (full_df['ex_id'] == 6))].copy()
    
    subjects = sorted(df['subject_id'].unique())
    results = []
    
    print(f"🚀 Starting v13 Mega LOSO Filtered (Excluding Fit3D Ex6) ({len(subjects)} subjects, {len(df)} frames)")
    ...
    res_df.to_csv(SAVE_DIR / "v13_loso_filtered_metrics.csv", index=False)
    
    summary = res_df.groupby(['view'])[['mae_raw', 'mae_model']].mean().reset_index()
    print("\n--- 🔥 v13 Mega Filtered (Fit3D Ex5 only) Results ---")
