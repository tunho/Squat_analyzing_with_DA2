...
def run_evaluation():
    ...
    for v_path in video_files:
        ...
        df['v6_corrected_angle'] = df['mp_knee_angle'] + model.predict(X)
        mae_raw = np.abs(df['gt_angle'] - df['mp_knee_angle']).mean()
        mae_v6 = np.abs(df['gt_angle'] - df['v6_corrected_angle']).mean()
        print(f"  ✅ {v_path.name}: Raw {mae_raw:.2f}° -> v6 {mae_v6:.2f}°")
        results.append({"video": v_path.name, "mae_raw": mae_raw, "mae_v6": mae_v6})
        pd.DataFrame(results).to_csv(PROJECT_ROOT / "outputs/ml_correction/v6_single_final/ex5_evaluation_v6.csv", index=False)
    print("\n✅ Ex5 v6 평가 완료")
