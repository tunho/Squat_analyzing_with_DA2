...
def get_leg_indices(subject_id):
    """Segmentation.csv에서 해당 피험자가 사용하는 다리 방향을 확인하여 인덱스 반환"""
    try:
        df_seg = pd.read_csv(SEGMENT_CSV, sep=';')
        # subject_id match (video_id starts with subject_id)
        sub_seg = df_seg[(df_seg['video_id'].str.startswith(subject_id)) & (df_seg['exercise_id'] == 5)]
        if sub_seg.empty:
...
def run_evaluation():
    ...
    print(f"\n🚀 [Ex5: 런지] v5 모델 성능 평가 시작 (총 {len(video_files)}개 영상)\n")

    for v_path in video_files:
        if "transposed" in v_path.name: # Skip side-view for now or handle appropriately
             # Actually handle both but we can skip if you want. Let's do both.
             pass

        sid = v_path.name.split('-')[0]
        suffix = "-".join(v_path.name.split('-')[1:]).replace(".mp4", "")
        ...
        h_idx, k_idx, a_idx = get_leg_indices(sid)
        try:
        ...
        pd.DataFrame(results).to_csv(PROJECT_ROOT / "outputs/ml_correction/v5_paper_final/ex5_evaluation_results.csv", index=False)
    ...
    print("\n" + "="*50)
    print(f"📊 [Ex5 Lunge] v5 모델 최종 평가 결과")
    print(f"  - 평균 Raw MAE: {avg_raw:.2f}°")
    print(f"  - 평균 v5 MAE:  {avg_v5:.2f}°")
    print(f"  - 전체 개선량:   {avg_raw - avg_v5:.2f}°")
    print(f"  - 개선율:       {(avg_raw-avg_v5)/avg_raw*100:.1f}%")
    print("="*50)
    
    report_df.to_csv(PROJECT_ROOT / "outputs/ml_correction/v5_paper_final/ex5_evaluation_results.csv", index=False)
    print(f"✅ 상세 결과 저장됨: {PROJECT_ROOT / 'outputs/ml_correction/v5_paper_final/ex5_evaluation_results.csv'}")
