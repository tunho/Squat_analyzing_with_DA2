...
| **공유/추정기고유 오차분해 + 전이** ("체계적오차 = 전이되는 입력난이도항 + 전이안되는 bias항") | **실행·반증됨(2026-07-02)**. 3추정기(nlf/rtmw3d/motionbert) 프레임정렬 OK(GT disagree 0.00). ①공유항 존재하나 약함: 추정기간 residual 상관 0.07~0.37. ②Leave-one-domain-out에서 공유항 cross-domain R² = 4개 중 +0.13/+0.27/−0.07/−0.43 (2양성 2음성, 비일관). ③공유항 보정 적용 시 4/4 도메인 backfire(도메인별 offset=전이안되는 bias항이 신호를 덮음). marginal/inconsistent 무덤. confound엔 안 걸렸으나 효과가 없음. |
| **스쿼트 폼 분류 (correct/incorrect)** | **실행·반증됨(2026-07-02)**. 발견: REHAB24-6 `Segmentation.csv`에 **correctness 라벨 내장**(전체 568정/504오답, Ex6 스쿼트=134정/61오답) + ex1/4/5는 error subtype도 있음. 그러나 **gold 3D GT 전신피처로도 Ex6 correctness 분류 LOSO AUC≈0.52~0.53, subject-norm 0.51 = chance.** 다리/스쿼트/런지(ex4/5/6) 모두 0.46~0.53. 팔운동(ex1/2/3)만 0.59~0.65 약신호(명확 subtype). 결론: **스쿼트 correctness는 운동학으로 결정 안 됨 → 완벽한 각도로도 분류 불가 → 우리 무릎각 자산과 연결 안 됨.** 라벨이 REHAB에만 있어 4도메인 강점도 못 씀. 전문가/외부분류기 붙여도 스쿼트에선 안 살아남. |
| **cross-domain offset을 관측특성(z 포함)으로 추정** | **실행·반증됨(2026-07-02)**. ①fps 기각: velocity=fps 함수(FiT3D 508프레임/rep→\|vel\|2.9 vs SUMe 11→22.0)이나, static-only LODO(REHAB 4.71→6.99)·프레임밀도 균등화 LODO(5.16→6.02) 둘 다 backfire 유지 → fps 아티팩트 아님. ②진짜 shift=depth영역(a_z 중앙값 REHAB −0.03 vs 타도메인 +0.13~0.36, 카메라 차이). ③z로 offset 추정 불가: within Spearman(resid,a_z) −0.09~−0.34(자세와 교락), 도메인간 corr 0.72지만 n=4 REHAB outlier, z-only LODO는 REHAB R²=−1.45 폭망. **결론: z는 도메인 마커지 residual 예측자 아님. offset은 실재하나 관측특성으로 식별 불가(unmeasured latent). 상세: `experiments/paper/clinical_utility/offset_estimation_findings.md`** |
